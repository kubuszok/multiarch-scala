...
// `multiarch-serviceloader` is NOT here: it has never been released, so there is no previous artifact
// to compare against. It joins this set in the release AFTER its first one, exactly as the plugin did.
...
lazy val root = project
  ...
  .aggregate(multiarchServiceLoader.projectRefs *)
  ...
  .settings(
    ...
    commands += Command.command("ci-release") { state =>
      ...
      // multiarch-resources and multiarch-serviceloader are projectMatrices: each JVM/JS/Native ×
      // Scala-version row is its own single-Scala-version project, so publish each row directly
      // (no `+` cross-build needed).
      def matrixRefs(refs: Seq[ProjectReference]): List[String] = refs.toList.map {
        case LocalProject(id) => s"$id/publishSigned"
        case ref              => s"${ref}/publishSigned"
      }
      val base =
        "+core/publishSigned" :: (matrixRefs(multiarchResources.projectRefs) ++
          matrixRefs(multiarchServiceLoader.projectRefs) ++ ("+plugin/publishSigned" ::
...
//
// `multiarch.serviceloader.PlatformServiceLoader` gives JVM/JS/Native a uniform stand-in for
// `java.util.ServiceLoader`, which the JVM resolves at run time, Scala.js does not have at all, and
// Scala Native exposes only through a link-time intrinsic that no generic wrapper can call (its
// `load` demands a literal `classOf`). Both non-JVM platforms therefore resolve providers by
// REGISTRATION, generated by `MultiArchServiceLoaderPlugin.embeddedServiceProvidersSettings` from
// the very same `META-INF/services/<service>` descriptors the JVM reads off the classpath.
//
// Both non-JVM axes here are pointed at this module's OWN TEST resources, so the one fixture
// descriptor drives all three mechanisms and the build-time half is proven end to end. The
// generator writes into `Test / sourceGenerators` (not `Compile`) so the fixture registrations stay
// out of the published artifact.
lazy val serviceLoaderScalaVersions = Seq("2.13.18", "3.3.8")

lazy val serviceLoaderTestProviders = MultiArchServiceLoaderPlugin.embeddedServiceProvidersSettings(
  resourceDir = Def.setting((Test / resourceDirectory).value),
  objectName = "multiarch.serviceloader.TestGeneratedServiceProviders",
  config = Test
)

lazy val multiarchServiceLoader = (projectMatrix in file("multiarch-serviceloader"))
  // JVM axis. JDK 26+ future-proofing: 3.3.8 enables the new lazy-vals encoding;
  // -Yfuture-lazy-vals requires -java-output-version >= 9. JVM-only (invalid on JS/Native).
  .jvmPlatform(
    scalaVersions = serviceLoaderScalaVersions,
    settings = Seq(
      scalacOptions ++= {
        if (scalaVersion.value == "3.3.8") Seq("-Yfuture-lazy-vals", "-java-output-version", "17")
        else Seq.empty
  ...
  // Scala.js axis: no java.util.ServiceLoader exists, so generate registration code from the test
  // descriptor (proving the build-time generator end to end).
  .jsPlatform(
    scalaVersions = serviceLoaderScalaVersions,
    settings = serviceLoaderTestProviders
  )
  // Scala Native axis: java.util.ServiceLoader exists but its `load` is a link-time intrinsic that
  // only accepts a literal classOf, so this axis registers from generated code exactly as JS does.
  .nativePlatform(
    scalaVersions = serviceLoaderScalaVersions,
    settings = serviceLoaderTestProviders ++ Seq(
      scalanative.sbtplugin.ScalaNativePlugin.autoImport.nativeConfig ~= {
